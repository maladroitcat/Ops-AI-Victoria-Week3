...
_DQ_REVALIDATE_INTERVAL_SECONDS = 300  # periodic re-check (5 min)
_DQ_TIMEOUT_SECONDS = 30
_DQ_STATUS = {
    "state": "partial",  # ready | partial | not_ready
...
def _execute_validation_and_mitigation() -> dict:
    """
    Run validation + mitigation and return readiness state.
    ready: no critical issues
    partial: timeout or non-critical issues
    not_ready: critical contract issues
    """
    def _aggregate_conflicting_duplicates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.warning(
        "[Week3 Data Quality] valid=%s issues=%s severities=%s",
        result["is_valid"],
        result["num_issues"],
        result.get("failed_severities", []),
    )
    for issue in result["issues"]:
        logger.warning("[Week3 Data Quality] issue=%s", issue)
    ...
    cleaned = current_df.copy()
    cleaned["time_bucket"] = pd.to_datetime(cleaned["time_bucket"])

    # 1) Duplicates: drop exact dupes, aggregate conflicting dupes by key.
    if "duplicate_keys" in issue_types and {"PULocationID", "time_bucket"}.issubset(cleaned.columns):
        before_rows = len(cleaned)
        dedup_exact = cleaned.drop_duplicates()
        exact_removed = before_rows - len(dedup_exact)
        if dedup_exact.duplicated(subset=["PULocationID", "time_bucket"], keep=False).any():
            dedup_final = _aggregate_conflicting_duplicates(dedup_exact)
            logger.warning(
                "[Week3 Data Quality] duplicate_conflicts_aggregated groups_resolved=%s",
                len(dedup_exact) - len(dedup_final),
            )
        else:
            dedup_final = dedup_exact
        logger.warning(
            "[Week3 Data Quality] duplicate_rows_removed exact_removed=%s total_removed=%s",
            exact_removed,
            len(cleaned) - len(dedup_final),
        )
        cleaned = dedup_final

    # 2) Out-of-range negatives and 3) high outliers: clip.
    if "trip_count" in cleaned.columns:
        baseline_upper = float(baseline_df["trip_count"].quantile(validator.thresholds.outlier_quantile))
        neg_before = int((cleaned["trip_count"] < 0).sum())
        hi_before = int((cleaned["trip_count"] > baseline_upper).sum())
        cleaned["trip_count"] = cleaned["trip_count"].clip(lower=0, upper=baseline_upper)
        logger.warning(
            "[Week3 Data Quality] trip_count_clipped negatives=%s high_outliers=%s upper=%.3f",
            neg_before,
            hi_before,
            baseline_upper,
        )

    # 4) Unexpected categorical rates: warn and proceed.
    if "holiday_rate_shift" in issue_types:
        logger.warning("[Week3 Data Quality] holiday rate shifted; proceeding with warning.")

    # 5) Feature no longer predicts: warn and fallback strategy marker.
    global _USE_BASELINE_FORECAST_FALLBACK
    _USE_BASELINE_FORECAST_FALLBACK = "lag_trip_correlation_shift" in issue_types
    if _USE_BASELINE_FORECAST_FALLBACK:
        logger.warning(
    ...
    state = "partial" if result["num_issues"] > 0 else "ready"
    reason = "non_critical_issues_present" if result["num_issues"] > 0 else "validated"
    return {
        "state": state,
        "reason": reason,
        "num_issues": result["num_issues"],
...
def _refresh_data_quality_status(force: bool = False) -> dict:
    """Periodic, timeout-bounded readiness refresh."""
    now = time.time()
    with _DQ_LOCK:
        if (not force) and (now - _DQ_STATUS["last_checked_ts"] < _DQ_REVALIDATE_INTERVAL_SECONDS):
            return dict(_DQ_STATUS)
        try:
            with ThreadPoolExecutor(max_workers=1) as ex:
                fut = ex.submit(_execute_validation_and_mitigation)
                status = fut.result(timeout=_DQ_TIMEOUT_SECONDS)
        except FutureTimeoutError:
            status = {
                "state": "partial",
                "reason": f"validation_timeout_gt_{_DQ_TIMEOUT_SECONDS}s",
                "num_issues": _DQ_STATUS.get("num_issues", 0),
            }
            logger.warning("[Week3 Data Quality] validation timed out (> %ss), marking partial-ready.", _DQ_TIMEOUT_SECONDS)
            ...
            logger.exception("[Week3 Data Quality] validation exception during readiness refresh: %s", exc)
        status["last_checked_ts"] = now
        _DQ_STATUS.update(status)
        return dict(_DQ_STATUS)


def get_data_quality_status() -> dict:
    """Expose current readiness status (with periodic revalidation)."""
    return _refresh_data_quality_status(force=False)


# Startup run: timeout-aware; partial-ready if slow.
_refresh_data_quality_status(force=True)


def _baseline_forecast_value(zone_id: int, hour: int, dow: int, holiday_name: str = "regular") -> float:
    """Fallback forecast from stable historical profile (zone/hour/day baseline)."""
    if holiday_name != "regular":
        sub = _profile[
            (_profile["PULocationID"] == zone_id)
            & (_profile["hour"] == hour)
            & (_profile["dayofweek"] == -1)
            & (_profile["holiday_name"] == holiday_name)
        ]
    else:
        sub = _profile[
            (_profile["PULocationID"] == zone_id)
    ...
    if len(sub) > 0:
        return float(sub["avg"].iloc[0])
    # Last-resort fallback when profile slice is missing
    zone_sub = _profile[(_profile["PULocationID"] == zone_id) & (_profile["holiday_name"] == "regular")]
    if len(zone_sub) > 0:
        return float(zone_sub["avg"].mean())
    return 0.0
...
def forecast_demand(zone_id: int, hour: int, dow: int, num_steps: int = 16, date: str = None) -> list:
    ...
    for step in range(num_steps):
        ...
        if _USE_BASELINE_FORECAST_FALLBACK:
            # Drift fallback: prefer stable historical baseline over lag-driven inference.
            pred_value = _baseline_forecast_value(
                zone_id=zone_id,
                hour=temporal_feats["hour"],
                dow=temporal_feats["dayofweek"],
                holiday_name="regular",
            )
        else:
